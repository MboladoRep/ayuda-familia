...
// Lista de fotos disponibles (las mismas que usa el generador)
...
export default function AdminPage() {
  ...
  const cancelarEdicion = () => {
    setEditandoId(null);
    setDatosEdicion({ titulo: '', contenido: '', imagen_url: '' });
  };

  const guardarCambios = async () => {
    try {
      const res = await fetch('/api/editar-articulo', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: editandoId, ...datosEdicion })
      });
      if (res.ok) {
    ...
    <main className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white p-8 rounded-xl shadow-lg mb-8">
          ...
          <button onClick={generarArticulo} className="bg-green-600 text-white font-bold py-3 px-6 rounded-lg hover:bg-green-700 transition mr-4">✨ Crear Nuevo Artículo</button>
                ...
                {editandoId === art.id ? (
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {FOTOS_DISPONIBLES.map((url, idx) => (
                          <div 
                            key={idx} 
                            onClick={() => setDatosEdicion({...datosEdicion, imagen_url: url})}
                            className={`cursor-pointer border-2 rounded overflow-hidden h-20 ${datosEdicion.imagen_url === url ? 'border-blue-600 ring-2 ring-blue-300' : 'border-transparent'}`}
                          >
                    ...
                    <div className="flex gap-2">
                      <button onClick={guardarCambios} className="bg-blue-600 text-white font-bold py-2 px-4 rounded hover:bg-blue-700">Guardar Cambios</button>
