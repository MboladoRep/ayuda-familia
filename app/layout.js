import './globals.css'

export const metadata = {
  // Título que saldrá en la pestaña y en WhatsApp
  title: 'Familia Ayuda | Tu red de apoyo en la crianza',
  
  // Descripción que saldrá debajo del título en WhatsApp
  description: 'Encuentra guías, consejos y recursos cerca de ti para resolver problemas de sueño, rabietas y educación. Asistente de IA incluido.',

  // Configuración para la previsualización en redes sociales (WhatsApp, Facebook, Twitter)
  openGraph: {
    title: 'Familia Ayuda | Crianza y Educación',
    description: 'Soluciones rápidas y mapa de recursos para familias.',
    url: 'https://ayuda-familia.vercel.app', // Cambia esto si tu dominio es distinto
    siteName: 'Familia Ayuda',
    images: [
      {
        // Usamos una imagen genérica de familia de Unsplash (gratis y sin derechos)
        url: 'https://images.unsplash.com/photo-1511895426328-dc8714191300?ixlib=rb-4.0.3&auto=format&fit=crop&w=1470&q=80',
        width: 1200,
        height: 630,
        alt: 'Familia feliz jugando juntos',
      },
    ],
    locale: 'es_ES',
    type: 'website',
  },
}

export default function RootLayout({ children) {
  return (
    <html lang="es">
      <body>{children}</body>
    </html>
  )
}
